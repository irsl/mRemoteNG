        ...
        public static void NavigateDocument(this DockPanel dockPanel, Direction direction, bool delegateDownToDockPanels = true)
        {
            ...
            if(info.SelectedDockContent != null)
            {
                if(delegateDownToDockPanels)
                {
                    ...
                    if ((info.SelectedDockContent is ConnectionWindow connectionWindow)&&(connectionWindow.Controls.Count > 0)&&(connectionWindow.Controls[0] is TabControl child))
                    {
                        int currentIndex = child.SelectedIndex;
                        var newTabIndex = currentIndex + (int)direction;
                        if ((newTabIndex > -1) && (newTabIndex < child.TabPages.Count))
                        {
                            // the desired new index belongs to the current dock panel, we can navigate internally.
                            child.SelectedIndex = newTabIndex;
                            return;
                        }
